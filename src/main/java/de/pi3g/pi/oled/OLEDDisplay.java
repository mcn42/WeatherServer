...
/**
 * A raspberry pi driver for the 128x64 pixel OLED display (i2c bus). The
 * supported kind of display uses the SSD1306 driver chip and is connected to
 * the raspberry's i2c bus (bus 1).
 * <p/>
 * Note that you need to enable i2c (using for example raspi-config). Also note
 * that you need to load the following kernel modules:
 * <pre>i2c-bcm2708</pre> and
 * <pre>i2c_dev</pre>
 * <p/>
 * Also note that it is possible to speed up the refresh rate of the display up
 * to ~60fps by adding the following to the config.txt of your raspberry:
 ...
 * </pre>
 * <p/>
 * This class is basically a rough port of Adafruit's BSD licensed SSD1306
...
public class OLEDDisplay {
    ...
    /**
     * creates an oled display object with default i2c bus 1 and the given
     * display address
     *
    ...
    public OLEDDisplay(int busNumber, int displayAddress) throws IOException {
        try { 
            LOGGER.info(String.format("Connecting to OLED Display at Bus %s, Address %s",busNumber,displayAddress));
            ...
            Logger.getLogger(OLEDDisplay.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
     ...
     * image to completely overwrite the current display content you need to
     * call clear() before.
     *
     * @param image
     * @param x
